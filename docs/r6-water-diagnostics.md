...
This candidate is the bounded **pre-Water scene-snapshot and draw-order proof** for R6. It still contains **no Classic replacement shader, refraction, reflection pass or ripple simulation**. The original colour-copy proof remains unchanged; an additional explicitly armed depth-transport diagnostic may copy the live pre-Water depth into R6-owned resources, but those resources are never sampled by a replacement in this candidate. When explicitly armed, it may perform up to eight colour-only same-frame render-target resolves solely to prove the producer/order seam. Native Wrath water is always rendered. R5 shadows, R4 grass/environment, R3 AO/SMAA, the fixed proxy and Wow.exe remain the baseline. No live validation is claimed before the home test.
...
| WXL_CLASSIC_WATER_DIAG_COPY | 0 | 0 observes ordering only; 1 arms the bounded colour pre-Water snapshot proof (maximum 8 process-wide, 4 per device generation); native rendering still continues |
| WXL_CLASSIC_WATER_DIAG_DEPTH_COPY | 0 | Requires COPY=1. 1 additionally tests the pre-Water two-stage depth transfer MSAA native depth -> single-sample native-format depth -> R6-owned INTZ. It never enables replacement or samples the result. |
...
The next-stage consumer policy is encoded but remains **observe-only**. Draw records classify a future replacement tranche using the exact recovered P01/P02/P03 keys: base Water only, pass 1, dominant zero-point-light VS `b3c587...`, shared Water PS `551eff...`, declaration `96f72c...`, exact P01/P02/P03 material hashes, and terrain/WMO provider identity. P05/P06 point-light variants, Magma, ProcWater, WaterNoSpec and every unknown profile remain native. A separate transport bit requires a valid same-scene, same-present-frame snapshot produced before the consumer on the same RT0. **`replacement_allowed` remains false regardless of these future-gate results.**
...
| draw | Liquid-scoped material/settings/provider/pass; shader SHA256s; declaration/FVF; topology/streams/indices; render states/hash; RT/depth/viewport; full-mode 16 pixel and 4 vertex texture/sampler stages and constant blocks; observe-only future P01/P02/P03 allow-list and snapshot-transport gate fields |
