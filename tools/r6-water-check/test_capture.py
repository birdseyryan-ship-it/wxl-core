...
class CaptureChecks(unittest.TestCase):
    def fixture(self):
        ...
        return [dict(base,event='session',sequence=1,copy_supported=True,replacement_supported=False,max_draws=2,copy_attempt_cap=8,copy_attempts_per_generation=4),
    ...
    def test_snapshot_valid(self):
        r=self.fixture()
        r.append(dict(r[0],event='snapshot_attempt',sequence=4,attempt_index=1,generation_attempt_index=1,replacement_allowed=False,
    ...
    def test_snapshot_cap(self):
        ...
        r.append(dict(r[0],event='snapshot_attempt',sequence=4,attempt_index=1,generation_attempt_index=1,replacement_allowed=False,attempted=True,snapshot_valid=False))
        with self.assertRaises(ValueError):self.run_records(r)
